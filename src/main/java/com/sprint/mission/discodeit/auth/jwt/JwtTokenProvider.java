...
public class JwtTokenProvider {
    ...
    public String generateAccessToken(Map<String, Object> claims,
                                      String subject) { // 받을 때 expiration, base64SecretKey 빠짐
        try {
            JWSSigner signer = new MACSigner(secretKey.getBytes(StandardCharsets.UTF_8));
            Date expiration = new Date(System.currentTimeMillis() + accessTokenExpirationMinutes * 60 * 1000); // 만료시간
            ...
            JWTClaimsSet claimsSet = new JWTClaimsSet.Builder()
                    ...
                    .issueTime(new Date())
                    .issuer("example.com") // example.com ? issuer이란?
                    .build();
    ...
    public String generateRefreshToken(String subject) {
        try {
            ...
            throw new RuntimeException("JWT RefreshToken 발급 실패", e);
        }
    }

    public Map<String, Object> getClaims(String token) {
        try{
            SignedJWT signedJWT = SignedJWT.parse(token); // 받은 token을 파싱
