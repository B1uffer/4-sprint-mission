...
public class BasicChannelService implements ChannelService {
  ...
  @PreAuthorize("hasRole('CHANNEL_MANAGER')") // 채널 생성할 때 CHANNEL_MANAGER 권한을 가져야함
  @Transactional
  @Override
  ...
  @PreAuthorize("hasRole('CHANNEL_MANAGER')") // CHANNEL_MANAGER 권한이 있어야함
  @Transactional
  @Override
  public ChannelDto update(UUID channelId, PublicChannelUpdateRequest request) { // 채널 수정
    ...
    Channel channel = channelRepository.findById(channelId)
        .orElseThrow(() -> ChannelNotFoundException.withId(channelId));
    if (channel.getType().equals(ChannelType.PRIVATE)) { // private일 경우
